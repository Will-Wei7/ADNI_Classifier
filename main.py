...
# MODEL_SAVE_PATH = 'adni_vit_classifier_best.pth' # Original state_dict save path
PEFT_MODEL_SAVE_PATH = 'adni_vit_lora_adapters' # Directory to save LoRA adapters
...
NUM_SLICES_PER_SCAN = 32 # Number of axial slices to extract from the middle of each scan
...
class ADNIDataset(Dataset):
    ...
    def __init__(self, data_dir, class_map, num_slices_per_scan, axis_to_slice, transform=None):
        ...
        Args:
            ...
            class_map (dict): Dictionary mapping class names (e.g., 'AD') to integers (e.g., 0).
        ...
        for class_name, label in self.class_map.items():
            ...
            # Use recursive glob to find files if they are nested deeper (optional)
        ...
        if not self.file_paths:
             raise ValueError(f"No NIfTI files found in the specified directory structure under {data_dir}")

        print(f"Total NIfTI files found: {len(self.file_paths)}")
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for inputs, labels in progress_bar:
        ...
        # --- !! Device Check Removed (was added for previous debug step) !! ---
...
def validate(model, loader, criterion, device):
    ...
    # Use average='weighted' to account for label imbalance in metrics
...
if __name__ == '__main__':
    ...
    try:
        ...
        lora_config = LoraConfig(
            ...
            # task_type=TaskType.IMAGE_CLASSIFICATION # Removed this line
        )
    ...
    for epoch in range(EPOCHS):
        ...
        if conf_matrix is not None:
            ...
            print("Labels:", [INV_CLASS_MAP[i] for i in range(NUM_CLASSES)])
            print(conf_matrix)
    ...
    if actual_epochs > 0:
        try:
            ...
            plt.tight_layout()
            plt.savefig('training_history_lora.png') # Changed filename
            print("Training history plot saved to training_history_lora.png")
    ...
    # Note: For evaluation, you might need to merge adapters for speed,
    # but loading them separately is standard for checking the trained state.
    if os.path.exists(PEFT_MODEL_SAVE_PATH):
        ...
        try:
            ...
            # Re-create the config used during training - REMOVED task_type here too
            ...
            # Make sure the adapter name matches if you used a custom one during save
    ...
    print("\n--- Test Set Results (Best LoRA Model) ---")
    if test_conf_matrix is not None:
        ...
        if test_labels_list: # Ensure there are samples
            try:
                ...
                print(f"Error calculating per-class test metrics: {e}")
        else:
            print("No valid samples in test set to calculate per-class metrics.")
    else:
